...
echo $before
hadoop jar $HADOOP_HOME/share/hadoop/tools/lib/hadoop-streaming-2.9.2.jar -D stream.non.zero.exit.is.failure=false -D mapreduce.input.fileinputformat.split.minsize=20971520 -D mapreduce.job.maps=10  -files /root/examples/avg/SumThreadPoolMapper.class,/root/examples/avg/SumReducer.class -input /user/root/input/mid-foo.txt -output /user/root/output-sum -mapper "java SumThreadPoolMapper 10" -reducer "java SumReducer"
