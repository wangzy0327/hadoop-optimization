...
if [ -z $a ]; then
   ...
   echo "not empty";
   printf "第"$i"次运行\t时间差:%0.9f s \t还差"$(($n-$i))"次\n" $difftime >> res-multifile-multiprocess.txt
 fi
