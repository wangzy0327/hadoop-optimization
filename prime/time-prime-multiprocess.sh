...
echo $before
hadoop jar $HADOOP_HOME/share/hadoop/tools/lib/hadoop-streaming-2.9.2.jar -D stream.non.zero.exit.is.failure=false -D mapred.min.split.size=104857600 -D mapred.map.tasks=20 -files /root/examples/prime/mapper-prime-multiprocess.py,/root/examples/prime/reducer.py -input /user/root/input/foo.txt -output /user/root/output-prime -mapper "python3 mapper-prime-multiprocess.py  " -reducer "python3 reducer.py"
...
a=`hdfs dfs -cat output-prime/*`
if [ -z "$a" ]
then
  echo "empty"
else
  echo "not empty"
  printf "第"$i"次运行\t时间差:%0.9f s \t还差"$(($n-$i))"次\n" $difftime >> res-prime-multiprocess.txt
fi
hdfs dfs -rm -r output-prime/
