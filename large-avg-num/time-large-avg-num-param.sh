...
echo $before
hadoop jar $HADOOP_HOME/share/hadoop/tools/lib/hadoop-streaming-2.9.2.jar -D stream.non.zero.exit.is.failure=false -D mapreduce.input.fileinputformat.split.minsize=62914560  -D mapreduce.job.maps=10 -files /root/examples/large-avg-num/SumMapper.class,/root/examples/large-avg-num/SumReducer.class -input /user/root/input/mid-foo.txt -output /user/root/output-sum -mapper "java SumMapper " -reducer "java SumReducer"
...
then
  ...
  echo "not empty"
  printf "第"$i"次运行sum\t时间差:%0.9f s \t还差"$(($n-$i))"次\n" $difftime >> res-sum-param.txt
fi
echo "sum : "$a
before1=`date -u "+%Y-%m-%d %H:%M:%S.%N"`
echo $before1
hadoop jar $HADOOP_HOME/share/hadoop/tools/lib/hadoop-streaming-2.9.2.jar -D stream.non.zero.exit.is.failure=false -D mapreduce.input.fileinputformat.split.minsize=62914560  -D mapreduce.job.maps=10  -files /root/examples/large-avg-num/CountMapper.class,/root/examples/large-avg-num/CountReducer.class -input /user/root/input/mid-foo.txt -output /user/root/output-count -mapper 'java CountMapper '$a'' -reducer 'java CountReducer'
...
echo $before2
hadoop jar $HADOOP_HOME/share/hadoop/tools/lib/hadoop-streaming-2.9.2.jar -D stream.non.zero.exit.is.failure=false -D mapreduce.input.fileinputformat.split.minsize=62914560  -D mapreduce.job.maps=10 -files /root/examples/large-avg-num/NumMapper.class,/root/examples/large-avg-num/NumReducer.class -input /user/root/input/mid-foo.txt -output /user/root/output-num -mapper 'java NumMapper '$c'' -reducer 'java NumReducer'
